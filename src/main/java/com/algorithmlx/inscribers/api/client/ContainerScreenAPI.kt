...
open class ContainerScreenAPI<T: Container>(
    pMenu: T,
    ...
    override fun renderBg(pMatrixStack: MatrixStack, pPartialTicks: Float, pX: Int, pY: Int) {
        this.getMinecraft().getTextureManager().bind(this.texture)
        val x = this.guiLeft
        val y = this.guiTop
        AbstractGui.blit(pMatrixStack, x, y, 0f, 0f, this.imageWidth, this.imageHeight, this.textureWidth, this.textureHeight)
    }
}